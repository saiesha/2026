...
export default function Home() {
  ...
  return <>
    <style>{`
      ...
      .closing-now { position:absolute; right:52px; top:52px; width:390px; padding:20px 22px 18px; border:1px solid rgba(16,24,39,.22); background:rgba(255,255,255,.38); color:#101827; }
      .now-head { display:flex; justify-content:space-between; align-items:center; padding-bottom:14px; border-bottom:1px solid rgba(16,24,39,.18); font-size:8px; font-weight:700; letter-spacing:.14em; text-transform:uppercase; }
      .now-head span:first-child { display:inline-flex; align-items:center; gap:7px; }
      .now-head span:first-child::before { content:""; width:6px; height:6px; border-radius:50%; background:#6841d1; }
      .now-head span:last-child { color:#667085; }
      .now-list { display:grid; }
      .now-item { display:grid; grid-template-columns:30px 1fr auto; gap:14px; align-items:center; min-height:58px; border-bottom:1px solid rgba(16,24,39,.13); }
      .now-item:last-child { border-bottom:0; }
      .now-item > span { color:#6841d1; font-size:8px; font-weight:700; }
      .now-item strong { font-size:15px; letter-spacing:-.03em; }
      .now-item em { font-style:normal; color:#667085; font-size:8px; letter-spacing:.08em; text-transform:uppercase; }
      .now-item:nth-child(2) > span { color:#d65ba7; }
      .now-item:nth-child(3) > span { color:#4f9d39; }
      .now-footer { padding-top:14px; color:#667085; font-size:8px; letter-spacing:.08em; text-transform:uppercase; }
      .now-item:hover strong { text-decoration:underline; text-decoration-color:#ab87ff; text-decoration-thickness:3px; text-underline-offset:4px; }
      ...
      html[data-theme="dark"] .closing-now { border-color:rgba(180,225,255,.22); background:#171c28; color:#f8f7f2; }
      html[data-theme="dark"] .now-head { border-color:rgba(180,225,255,.18); }
      html[data-theme="dark"] .now-head span:first-child::before { background:#b4e1ff; }
      html[data-theme="dark"] .now-head span:last-child,
      html[data-theme="dark"] .now-item em,
      html[data-theme="dark"] .now-footer { color:#aeb5c2; }
      html[data-theme="dark"] .now-item { border-color:rgba(180,225,255,.14); }
      html[data-theme="dark"] .now-item > span { color:#c7b4ff; }
      html[data-theme="dark"] .now-item:nth-child(2) > span { color:#fface4; }
      html[data-theme="dark"] .now-item:nth-child(3) > span { color:#c1ff9b; }
      html[data-theme="dark"] .now-item:hover strong { text-decoration-color:#b4e1ff; }
      ...
      @media (max-width: 700px) {
        .closing { padding:42px 28px; min-height:510px; }
        .closing-now { position:relative; right:auto; top:auto; width:auto; margin:0 0 42px; padding:17px 18px 15px; }
        .now-item { min-height:50px; grid-template-columns:25px 1fr; gap:10px; }
        .now-item em { grid-column:2; margin-top:-7px; }
        .now-footer { font-size:7px; }
      }
    `}</style>
    <main>
